...
const SolicitudDiferencias = () => {
  ...
  // Acción para actualizar o vincular desde el detalle
  const handleVincularOrdenCompra = ({ ordenCompra, detallesActualizados, estadoGeneral }) => {
    ...
    confirmAction(
      "Confirmar Actualización de Solicitud",
      `¿Desea actualizar la gestión de diferencias con la Orden de Compra N° ${folioOC}?`,
      async () => {
        try {
          ...
          try {
            ...
            await addDoc(logsRef, {
              accion: "GESTION_DIFERENCIAS",
              detalle: `Revisión/actualización de diferencias con OC N° ${folioOC} sobre folio ${facturaSeleccionada.folio || facturaSeleccionada.id} — Resultado: ${estadoFinal}`,
            ...
            console.error("Error al registrar log de diferencias:", logError);
          }

          showToast(`Solicitud de diferencias actualizada — ${estadoFinal}`, "success");
          ...
          // Si el estado cambia a otro que ya no es diferencia, remover de la lista
          if (!["Diferencia Reportada", "DiferenciasReportadas", "Aceptado con Diferencias"].includes(estadoFinal)) {
            setFacturas(prev => prev.filter(f => f.id !== facturaSeleccionada.id));
          }
        } catch (error) {
          console.error("Error al actualizar la solicitud de diferencias:", error);
          showToast("Error al procesar los cambios de la solicitud", "error");
        }
      }
    );
  };
