...
const MODULO_ACTUAL = 'hemodinamia';

const ALL_TABS = [
  { id: 'detalles', label: 'Detalles', Icon: ListFilter, path: '/hemodinamia/gestionHemodinamia/detalles' },
  { id: 'informacion', label: 'Información', Icon: Info, path: '/hemodinamia/gestionHemodinamia/informacion' },
  { id: 'cargas', label: 'Cargas', Icon: UploadCloud, path: '/hemodinamia/gestionHemodinamia/cargas' },
  { id: 'logs', label: 'Logs', Icon: History, path: '/hemodinamia/gestionHemodinamia/logs' },
];

const GestionesHemodinamiaDetalleView = forwardRef(({
  item,
  todosLosRegistros = [],
  onGuardar,
  onCancelar,
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => {
        console.error('Error al escuchar el período activo de Hemodinamia:', error);
        setCargandoPeriodo(false);
      }
    );
    return () => unsub();
      ...
      {!cargandoPeriodo && (
        periodoActivo ? (
          <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 text-[10px] text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border-b border-emerald-100 dark:border-emerald-900/40">
            ...
            Período abierto para Hemodinamia: <strong>{nombrePeriodoActivo}</strong>
          </div>
        ) : (
          <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 text-[10px] text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-900/40">
            ...
            No hay un período abierto para Hemodinamia.
          </div>
        )
      )}
...
export default GestionesHemodinamiaDetalleView;
