...
import { CENTRO_HEMODINAMIA } from './constantesHemodinamia';
...
export const exportarGestionesAExcel = (registros, showToast) => {
  if (!registros || registros.length === 0) {
    if (showToast) showToast("No hay datos para exportar", "error");
    return;
  }

  const dataExportar = registros.map(i => ({
  ...
  XLSX.writeFile(workbook, `gestiones_hemodinamia_${fechaHoy}.xlsx`);
...
export const descargarPlantillaCSV = (showToast) => {
  ...
  const ejemplo = ["123456", "Juan Pérez", "2026-09-15", "MEDTRONIC", "PENDIENTE", CENTRO_HEMODINAMIA, "HEMODINAMIA", "AGENDANDO", "1500000"];
  ...
  link.setAttribute("download", "plantilla_gestiones_hemodinamia.csv");
...
const normalizarYValidarDatos = (rows) => {
  return rows.map(row => {
    ...
    return {
      ...
      atributo: (getVal('ATRIBUTO') || 'HEMODINAMIA').toUpperCase(),
