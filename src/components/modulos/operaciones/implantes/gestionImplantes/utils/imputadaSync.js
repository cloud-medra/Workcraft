...
// Extraído de guardarDesdeDetalle (useGestionesImplantesData.js) para que la
// herramienta de Sincronización de Imputadas reutilice EXACTAMENTE el mismo
// mapeo de campos que ya usa el candado al re-sincronizar un bloque
// SOLICITADO — un solo lugar define la forma del documento en
// implantes_imputadas, en vez de mantener dos copias que puedan divergir.
...
export const refImputada = (periodoAnio, periodoMes, itemId) =>
  doc(db, 'implantes_imputadas', String(periodoAnio), 'meses', periodoMes, 'documentos', itemId);

/**
 * Construye el documento de implantes_imputadas correspondiente a un ítem
 * de implantes_gestiones. `dataGestion` es el objeto ya normalizado del
