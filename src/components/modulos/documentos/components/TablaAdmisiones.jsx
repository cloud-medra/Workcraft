      ...
      <table className="w-full text-left text-[11px] border-collapse">
        <thead className="bg-gray-100 sticky top-0">
          <tr className="text-gray-600 uppercase font-bold">
            ...
            {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colEstadoOC') && <th className="p-2 border-b border-r border-gray-200 text-center">Estado OC</th>}
            {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colTotal') && <th className="p-2 border-b border-r border-gray-200 text-right">Total</th>}
            {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colEstadoGral') && <th className="p-2 border-b border-r border-gray-200 text-center">Estado General</th>}
            {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_btnVer') && <th className="p-2 border-b border-gray-200 text-center">Acciones</th>}
          </tr>
        </thead>
        <tbody>
          {datos.map((d) => {
            ...
            return (
              ...
              >
                ...
                {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colEstadoOC') && (
                  <td className="p-2 border-b border-r border-gray-200 text-center">
                    {(() => {
                      ...
                      if (cantOC === 0) return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-red-100 text-red-700">SIN OC</span>;
                      if (cantOC < cantProveedores) {
                        return (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-yellow-100 text-yellow-700" title="Faltan OC de este expediente">
                            PARCIAL ({cantOC}/{cantProveedores})
                          </span>
                        );
                      }
                      return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-green-100 text-green-700">CON OC</span>;
                    })()}
                  </td>
                )}

                {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colTotal') && (
                  <td className="p-2 border-b border-r border-gray-200 text-right font-medium text-gray-700">
                    {totalFormateado}
                  </td>
                )}
                ...
                {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_colEstadoGral') && (
                  <td className="p-2 border-b border-r border-gray-200 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${String(d.ESTADO_GENERAL).trim().toUpperCase() === 'FACTURADO' ? 'bg-green-100 text-green-700' :
                        String(d.ESTADO_GENERAL).trim().toUpperCase() === 'PENDIENTE' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'
                      }`}>
                ...
                {hasPermission(RUTA, 'tabla_admisiones', 'cargaDatos_tabla_btnVer') && (
                  <td className="p-2 border-b border-gray-200 text-center">
                    <button onClick={() => onSeleccionarFila(d)} className="text-green-600 hover:text-green-800" title="Visualizar expediente">
                      <Eye size={14} />
                    </button>
                  </td>
                )}
