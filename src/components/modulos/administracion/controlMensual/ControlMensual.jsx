...
const ControlMensual = () => {
  ...
  // Hook personalizado de lógica y Firebase
  const {
    estadosModulos,
    resumenImputaciones,
    cargando,
  ...
  // INSPECCIÓN DE ESTADOS DE MÓDULOS PARA BLOQUEO DE AÑO
  ...
  const mesesIniciados = MESES.filter((mes, index) => {
    const numeroMesStr = String(index + 1).padStart(2, '0'); // '01', '02', etc.
    const numeroMesAlt = String(index + 1);                    // '1', '2', etc.

    return modulosVisibles.some(mod => {
      const datosModulo = estadosModulos[mod.id] || {};
      // Comprobamos tanto con el id original como con las alternativas normalizadas
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-lg shadow-xs overflow-hidden font-sans relative">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 dark:bg-gray-900 border-b border-slate-200 dark:border-gray-700 text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-gray-200">
                  <th rowSpan={2} className="py-2.5 px-4 border-r border-slate-200 dark:border-gray-700 bg-slate-100 dark:bg-gray-900 sticky left-0 z-10 w-48">
                    Período
                  </th>
                ...
                <tr className="bg-slate-50 dark:bg-gray-900/60 border-b border-slate-200 dark:border-gray-700 text-[9px] font-bold uppercase tracking-wider text-slate-500 dark:text-gray-400">
                  {modulosVisibles.map((mod) => (
                    <React.Fragment key={`sub_${mod.id}`}>
                      <th className="py-1.5 px-2 text-center border-r border-slate-100 dark:border-gray-800 w-32">Estado / Acción</th>
                      <th className="py-1.5 px-2 text-right border-r border-slate-100 dark:border-gray-800 w-16">Docs</th>
                      <th className="py-1.5 px-2 text-right border-r border-slate-200 dark:border-gray-700 w-28">Monto Total</th>
                    </React.Fragment>
                  ))}
                </tr>
              </thead>
                ...
                {mesesIniciados.map((mes, index) => {
                  ...
                  return (
                    <tr key={mes.id || index} className="hover:bg-slate-50/80 dark:hover:bg-gray-800/50 transition-colors">
                      {/* COLUMNA FIJA DE PERÍODO */}
                      <td className="py-2.5 px-4 font-bold text-slate-800 dark:text-gray-100 border-r border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 sticky left-0 z-10">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            className="px-1.5 py-0.5 bg-slate-100 hover:bg-slate-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-slate-600 dark:text-gray-300 rounded text-[9px] font-bold flex items-center gap-1 transition cursor-pointer disabled:opacity-50 shrink-0"
                          >
                      ...
                      {modulosVisibles.map((mod) => {
                        ...
                        return (
                          <React.Fragment key={`${mes.id}_${mod.id}`}>
                            {/* ESTADO Y BOTONES EN LÍNEA */}
                            <td className="py-2 px-2 border-r border-slate-100 dark:border-gray-800 align-middle">
                                ...
                                <div className="flex items-center gap-0.5">
                                  {estado === 'SIN_INICIAR' && (
                                    <button
                                      ...
                                      className="p-1 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 rounded transition cursor-pointer disabled:opacity-50"
                                    >
                                      <PlayCircle size={14} />
                                    </button>
                                  )}
                                  ...
                                  {(estado === 'ABIERTO' || estado === 'REABIERTO') && (
                                    <button
                                      ...
                                      className="p-1 text-slate-500 hover:text-slate-800 dark:hover:text-gray-200 hover:bg-slate-100 dark:hover:bg-gray-700 rounded transition cursor-pointer disabled:opacity-50"
                                    >
                                      <Lock size={14} />
                                    </button>
                                  )}
                                  ...
                                  {estado === 'CERRADO' && (
                                    <button
                                      ...
                                      className="p-1 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/50 rounded transition cursor-pointer disabled:opacity-50"
                                    >
                            ...
                            {/* DOCUMENTOS */}
                            <td className="py-2 px-2 border-r border-slate-100 dark:border-gray-800 text-right font-mono text-slate-700 dark:text-gray-300 align-middle">
                              {infoImputacion.cantidad.toLocaleString('es-CL')}
                            </td>

                            {/* MONTO TOTAL */}
                            <td className="py-2 px-2 border-r border-slate-200 dark:border-gray-700 text-right font-mono font-semibold text-slate-800 dark:text-gray-200 align-middle">
      ...
      {/* MODALES REAPERTURA E HISTORIAL */}
      <ModalReapertura
