...
const COL_DOCUMENTOS = "laboratorio_documentos";
const ESTADOS_EXCLUIDOS = ["Rechazada"];
// Suman igual, pero el monto es "temporal": hay una diferencia por resolver en la OC.
const ESTADOS_TEMPORALES = ["Diferencia Reportada", "Solicitud Enviada"];
// Estados definitivos: sin diferencias (Listo para Ingreso; Finalizado = ya ingresada/imputada).
const ESTADOS_FINALES = ["Listo para Ingreso", "Listos para Ingreso", "Listo Para Ingreso", "Finalizado"];
...
/**
 * Acumulado facturado por código de una orden. Se calcula siempre desde las facturas
 ...
 */
export const useFacturacionOrden = (ordenId) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snap) => {
            ...
            snap.forEach((d) => {
                ...
                if (d.ref.path.split('/')[0] !== COL_DOCUMENTOS) return;
                const data = d.data();
                if (ESTADOS_EXCLUIDOS.includes(data.estado)) return;
                ...
                (data.detalles || []).forEach((item) => {
                    ...
                    else entrada.documentos.push({ id: d.id, folio, cantidad, estado: data.estado || '', temporal: ESTADOS_TEMPORALES.includes(data.estado) });
                });
            });

            Object.values(acumulado).forEach((e) => {
                e.tieneTemporal = e.documentos.some(x => x.temporal);
                e.todosFinales = e.documentos.length > 0 && e.documentos.every(x => ESTADOS_FINALES.includes(x.estado));
            });
