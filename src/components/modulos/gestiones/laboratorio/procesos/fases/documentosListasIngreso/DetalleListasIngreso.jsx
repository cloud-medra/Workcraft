...
const DetalleListasIngreso = ({
  ...
  const { periodo: periodoAbierto, cargando: cargandoPeriodo } = usePeriodoAbierto('laboratorio');
  ...
  const handleFinalizarActa = async () => {
    ...
    try {
      ...
      const periodo = await obtenerPeriodoAbierto('laboratorio');
      if (!periodo) {
        showToast('No hay un período de imputación abierto para Laboratorio. No se puede finalizar.', 'error');
        return;
      }
      ...
      const refAnio = doc(db, 'laboratorio_imputadas', periodo.anio);
      const refMes = doc(db, 'laboratorio_imputadas', periodo.anio, 'meses', periodo.mes);
          ...
          <div className="w-full max-w-sm h-full bg-white dark:bg-gray-800 border-l border-slate-200 dark:border-gray-700 shadow-xl flex flex-col justify-between p-4 animate-in slide-in-from-right duration-200">
            ...
            {cargandoPeriodo ? null : periodoAbierto ? (
              ...
              <div className="p-2.5 rounded border border-rose-200 dark:border-rose-800/50 bg-rose-50 dark:bg-rose-950/30 text-[11px] text-rose-700 dark:text-rose-300 flex items-start gap-2">
                ...
                <p>No hay un período de imputación abierto para Laboratorio. No se puede finalizar hasta que se abra uno.</p>
              </div>
            )}
