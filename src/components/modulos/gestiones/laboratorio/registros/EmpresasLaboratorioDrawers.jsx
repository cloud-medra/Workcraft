      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
        {loadingLogs ? (
          ...
          <div className="text-center py-12 text-gray-400 dark:text-gray-500 text-[10px]">
            No hay registros de auditoría para este laboratorio.
          </div>
        ) : (
          logsList.map((log) => (
            <div
...
// pathVista: PATH_VISTA de la vista dueña de este drawer (ej: "/laboratorio/empresas").
// Se pasa como prop en vez de importarlo fijo para que este mismo componente
// pueda reutilizarse desde Vacunatorio u otro módulo con su propio PATH_VISTA.
export const ConfigDrawer = ({
  ...
  const SECCION = 'drawer_configuracion';

  const puedeExportar = hasPermission(pathVista, SECCION, 'btn_exportar');
  const puedeDescargarPlantilla = hasPermission(pathVista, SECCION, 'btn_descargar_plantilla');
  const puedeSeleccionarArchivo = hasPermission(pathVista, SECCION, 'input_archivo');
  const puedeImportar = hasPermission(pathVista, SECCION, 'btn_importar');

  // Si no tiene permiso para nada dentro de este drawer, no vale la pena
  // mostrar la sección de importación completa (ni siquiera el bloque vacío).
  const tieneAlgoDeImportacion =
    puedeDescargarPlantilla || puedeSeleccionarArchivo || puedeImportar;
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        {puedeExportar && (
          <div className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50/50 dark:bg-gray-900/30 space-y-2">
            <div className="flex items-center gap-2 text-gray-800 dark:text-gray-200 font-bold text-[11px]">
              ...
              <span>Exportar Laboratorios</span>
            </div>
            <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed">
              Descarga la lista actual de laboratorios registrados ({totalLaboratorios}{' '}
        ...
        {tieneAlgoDeImportacion && (
          <div className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50/50 dark:bg-gray-900/30 space-y-2.5">
            ...
            <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed">
              Carga masivamente nuevos laboratorios seleccionando un archivo
        ...
        {!puedeExportar && !tieneAlgoDeImportacion && (
          <div className="text-center py-10 text-gray-400 dark:text-gray-500 text-[10px]">
            No tienes permisos habilitados dentro de esta sección.
          </div>
        )}
      </div>
