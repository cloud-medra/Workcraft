...
import { DrawersOverlay, LogDrawer, ConfigDrawer } from './EmpresasLaboratorioDrawers';
...
const EmpresasLaboratorios = () => {
  ...
  const handleExportarDatos = () => {
    ...
    const dataExportar = laboratorios.map(l => ({
      NOMBRE: l.nombre || '',
      RUT: l.rut || '',
    ...
    XLSX.writeFile(workbook, `empresas_laboratorios_${new Date().toISOString().slice(0,10)}.xlsx`);
  ...
  const handleDescargarPlantilla = () => {
    ...
    const ejemplo = ["Laboratorio Central", "12345678-9", "ACTIVO"];
    ...
    link.setAttribute("download", "plantilla_importacion_laboratorios.csv");
  ...
  const filtrarDuplicados = (registros) => {
    const rutsExistentes = new Set(laboratorios.map(l => l.rut));
    const nombresExistentes = new Set(laboratorios.map(l => l.nombre.trim().toLowerCase()));
    ...
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden p-0 relative text-[11px]">
      ...
      <div className="px-3 py-2 flex items-center justify-between border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/80">
        <h2 className="text-[12px] font-bold text-gray-700 dark:text-gray-100 flex items-center gap-1.5">
          ...
          {editingId ? "EDITAR LABORATORIO" : "REGISTRO DE LABORATORIOS"}
        </h2>
        ...
        {hasPermission(PATH_VISTA, "header", "btn_configuracion") && (
          <button
            ...
            title="Configuración de Laboratorios (Importar/Exportar)"
          >
            <Settings size={15} />
          </button>
        )}
