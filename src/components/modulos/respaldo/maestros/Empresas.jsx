...
import { Building2, Plus, Trash2, Search, RefreshCw, Pencil, Save, X } from 'lucide-react';
...
const Empresas = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, "maestros_empresas"), orderBy("fechaRegistro", "asc"));
  ...
  const handleGuardar = async (e) => {
    ...
    const dataAEnviar = {
      ...
      fechaRegistro: editingId ? (empresas.find(e => e.id === editingId)?.fechaRegistro || new Date()) : new Date(),
      registradoPor: userData?.nombreCompleto || 'Usuario'
    };

    try {
      if (editingId) {
        await updateDoc(doc(db, "maestros_empresas", editingId), dataAEnviar);
        showToast("Empresa actualizada correctamente", "success");
      } else {
        await addDoc(collection(db, "maestros_empresas"), dataAEnviar);
  ...
  const handleDelete = (id) => {
    confirmAction(
      ...
      async () => {
        try {
          await deleteDoc(doc(db, "maestros_empresas", id));
  ...
  const iniciarEdicion = (e) => {
    setEditingId(e.id);
    setFormData({ nombre: e.nombre, rut: e.rut, estado: e.estado || 'ACTIVO' });
  };
        ...
        <form onSubmit={handleGuardar} className="p-4 flex flex-wrap items-end gap-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-800/20">
          {hasPermission(PATH_VISTA, "formulario_registro", "input_nombre") && (
            <div className="w-[300px]">
              <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Nombre Empresa</label>
              <input required value={formData.nombre} onChange={e => setFormData({ ...formData, nombre: e.target.value })} className="w-full h-8 px-2 border border-gray-300 dark:border-gray-600 rounded text-[12px] outline-none focus:border-[#2383C2] dark:focus:border-[#2383C2] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100" placeholder="Ej: Laboratorio X" />
            </div>
          )}
          ...
          <table className="w-full text-left text-[12px] border-collapse">
            ...
            <tbody>
              {empresasFiltradas.map((e, index) => (
                <tr key={e.id} className="border-l-4 border-transparent hover:border-[#2383C2] hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors">
                  ...
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_nombre") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-700 dark:text-gray-200 font-medium">{e.nombre}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_rut") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-600 dark:text-gray-300">{e.rut}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_estado") && (
                    <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${e.estado === 'INACTIVO' ? 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400' : 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-400'}`}>
                        {e.estado || 'ACTIVO'}
                      </span>
                    </td>
                  )}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_registrador") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{e.registradoPor || 'N/A'}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_fecha") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{formatearFecha(e.fechaRegistro)}</td>}
                  ...
                  <td className="p-3 border-b border-gray-200 dark:border-gray-700 text-center">
                    <div className="flex justify-center gap-3">
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_editar") && (
                        <button onClick={() => iniciarEdicion(e)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition"><Pencil size={15} /></button>
                      )}
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_eliminar") && (
                        <button onClick={() => handleDelete(e.id)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition"><Trash2 size={15} /></button>
                      )}
                    </div>
                  </td>
                </tr>
