// src/components/facturas/EmpresasHemodinamia.jsx
...
const EmpresasHemodinamia = () => {
  const [hemodinamias, setHemodinamias] = useState([]);
  ...
  const [selectedHemodinamiaForLog, setSelectedHemodinamiaForLog] = useState(null);
  ...
  const PATH_VISTA = "/hemodinamia/empresas";
  const COL_BASE = "hemodinamia_empresas";

  useEffect(() => {
    const q = query(collection(db, COL_BASE), orderBy("fechaRegistro", "asc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setHemodinamias(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    return () => unsubscribe();
  }, []);
  ...
  const registrarLog = async (hemodinamiaId, accion, detalles) => {
    try {
      const logsSubcollectionRef = collection(db, COL_BASE, hemodinamiaId, "logs");
  ...
  const handleGuardar = async (e) => {
    ...
    const existeRut = hemodinamias.some(h => 
      h.rut === rutFormateado && h.id !== editingId
    );
    if (existeRut) {
      return showToast("Ya existe una hemodinamia registrada con este RUT", "error");
    }

    // 2. Validar Nombre duplicado (excluyendo el registro actual si se está editando)
    const existeNombre = hemodinamias.some(h => 
      h.nombre.trim().toLowerCase() === nombreTrimmed && h.id !== editingId
    );
    if (existeNombre) {
      return showToast("Ya existe una hemodinamia registrada con este Nombre", "error");
    }

    try {
      if (editingId) {
        const hemodinamiaExistente = hemodinamias.find(h => h.id === editingId);
        
        const dataAEnviar = {
          ...formData,
          rut: rutFormateado,
          fechaRegistro: hemodinamiaExistente?.fechaRegistro || new Date(),
          registradoPor: hemodinamiaExistente?.registradoPor || userData?.nombreCompleto || 'Usuario'
        };

        await updateDoc(doc(db, COL_BASE, editingId), dataAEnviar);

        await registrarLog(editingId, 'EDICION', {
          nombreAnterior: hemodinamiaExistente?.nombre,
          nombreNuevo: formData.nombre,
          rutAnterior: hemodinamiaExistente?.rut,
          rutNuevo: rutFormateado,
          estadoAnterior: hemodinamiaExistente?.estado,
          estadoNuevo: formData.estado
        });

        showToast("Hemodinamia actualizada correctamente", "success");
      } else {
        const dataAEnviar = {
          ...formData,
          rut: rutFormateado,
        ...
        showToast("Hemodinamia registrada correctamente", "success");
      }
  ...
  const handleDelete = (id) => {
    const hemodinamiaAEliminar = hemodinamias.find(h => h.id === id);

    confirmAction(
      "Eliminar Hemodinamia",
      ...
      async () => {
        try {
          ...
          await registrarLog(id, 'ELIMINACION', {
            nombre: hemodinamiaAEliminar?.nombre || '',
            rut: hemodinamiaAEliminar?.rut || ''
          });

          await deleteDoc(doc(db, COL_BASE, id));

          showToast("Hemodinamia eliminada correctamente", "info");
        } catch (error) {
  ...
  const iniciarEdicion = (h) => {
    setEditingId(h.id);
    setFormData({ nombre: h.nombre, rut: h.rut, estado: h.estado || 'ACTIVO' });
  };

  const abrirHistorialLogs = async (hemodinamia) => {
    setSelectedHemodinamiaForLog(hemodinamia);
    setShowLogModal(true);
    setLoadingLogs(true);

    try {
      // Consulta a la subcolección interna: hemodinamia_empresas/{hemodinamia.id}/logs
      const q = query(
        collection(db, COL_BASE, hemodinamia.id, "logs"),
        orderBy("fecha", "desc")
      );
  ...
  const hemodinamiasFiltradas = hemodinamias.filter(h =>
    h.nombre.toLowerCase().includes(busqueda.toLowerCase()) ||
    (h.rut && h.rut.toLowerCase().includes(busqueda.toLowerCase()))
  );
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden p-0 relative text-[11px]">
      <h2 className="text-[12px] font-bold text-gray-700 dark:text-gray-100 px-3 py-2 flex items-center gap-1.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/80">
        <Microscope size={14} className="text-[#2383C2]" /> {editingId ? "EDITAR HEMODINAMIA" : "REGISTRO DE HEMODINAMIA"}
      </h2>
        ...
        <form onSubmit={handleGuardar} className="px-3 py-2 flex flex-wrap items-end gap-2.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-800/20">
          {hasPermission(PATH_VISTA, "formulario_registro", "input_nombre") && (
            <div className="w-[240px]">
              <label className="block text-[9px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Nombre Hemodinamia</label>
              <input required value={formData.nombre} onChange={e => setFormData({ ...formData, nombre: e.target.value })} className="w-full h-7 px-2 border border-gray-300 dark:border-gray-600 rounded text-[11px] outline-none focus:border-[#2383C2] dark:focus:border-[#2383C2] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100" placeholder="Ej: Hemodinamia Central" />
            </div>
          )}
      ...
      {hasPermission(PATH_VISTA, "tabla_datos") && (
        <div className="flex-grow overflow-auto">
          <table className="w-full text-left text-[11px] border-collapse">
            ...
            <tbody>
              {hemodinamiasFiltradas.map((h, index) => (
                <tr key={h.id} className="border-l-2 border-transparent hover:border-[#2383C2] hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors">
                  ...
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_nombre") && <td className="py-1 px-2 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-700 dark:text-gray-200 font-medium">{h.nombre}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_rut") && <td className="py-1 px-2 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-600 dark:text-gray-300">{h.rut}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_estado") && (
                    <td className="py-1 px-2 border-b border-r border-gray-200 dark:border-gray-700/70">
                      <span className={`text-[9px] px-1.5 py-0.2 rounded-full font-bold uppercase ${h.estado === 'INACTIVO' ? 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400' : 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-400'}`}>
                        {h.estado || 'ACTIVO'}
                      </span>
                    </td>
                  )}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_registrador") && <td className="py-1 px-2 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{h.registradoPor || 'N/A'}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_fecha") && <td className="py-1 px-2 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{formatearFecha(h.fechaRegistro)}</td>}
                  ...
                  <td className="py-1 px-2 border-b border-gray-200 dark:border-gray-700 text-center">
                    <div className="flex justify-center gap-2">
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_log") && (
                        <button onClick={() => abrirHistorialLogs(h)} title="Ver Historial / Logs" className="text-gray-500 hover:text-[#2383C2] dark:hover:text-[#2383C2] transition">
                          <History size={13} />
                        </button>
                      )}
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_editar") && (
                        <button onClick={() => iniciarEdicion(h)} title="Editar" className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition">
                          <Pencil size={13} />
                        </button>
                      )}
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_eliminar") && (
                        <button onClick={() => handleDelete(h.id)} title="Eliminar" className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition">
                          <Trash2 size={13} />
                        </button>
                      )}
                    </div>
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <div className="truncate">
              ...
              <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate">
                {selectedHemodinamiaForLog?.nombre}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loadingLogs ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 text-gray-400 dark:text-gray-500 text-center px-4">
              ...
              <p className="text-[10px]">No hay actividad documentada para esta hemodinamia.</p>
            </div>
          ) : (
            logsList.map((log) => (
...
export default EmpresasHemodinamia;