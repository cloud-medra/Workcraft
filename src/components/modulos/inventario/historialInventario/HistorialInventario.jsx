...
const HistorialInventario = () => {
  ...
  useEffect(() => {
    // Ordenamos por la fecha real que maneja tu estructura: fechaEfectivaEgreso
    const qHistorial = query(collection(db, COL_EGRESOS), orderBy("fechaEfectivaEgreso", "desc"));
  ...
  const filas = useMemo(() => {
    ...
    historialDocs.forEach((docItem) => {
      // Leemos la fecha efectiva del egreso
      ...
      // Cambiamos de 'items' a 'itemsEgresados' según tu estructura en Firestore
    ...
    return resultado;
  }, [historialDocs, filtroTexto, fechaDesde, fechaHasta]);
