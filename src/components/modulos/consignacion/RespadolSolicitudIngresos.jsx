...
import { FilePlus, Save, CheckCircle, AlertCircle, Edit2, X, Trash2, Sidebar, Info } from 'lucide-react';
...
const SolicitudIngresos = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (sidebarRef.current  && !sidebarRef.current.contains(event.target) && !event.target.closest('.action-btn') && !event.target.closest('tr')) {
  ...
  const handleIniciarEdicionFormulario = (solicitud, e) => {
  ...
  const handleEliminarFila = async (refPath, e) => {
    if (e) e.stopPropagation();
  ...
  const handleIniciarAsignacion = (solicitud, e) => {
    if (e) e.stopPropagation();
  ...
  const handleGuardarCodigoNuevo = async (refPath, e) => {
    if (e) e.stopPropagation();
  ...
  return (
    <div className="w-full h-full flex flex-row bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden relative transition-colors">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-700 dark:text-gray-200">
              {solicitudes.map((s, index) => (
                <tr
                  ...
                  className={`border-l-4 border-transparent cursor-pointer hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors ${editandoId === s.id ? 'bg-amber-50/20 dark:bg-amber-950/10 border-l-amber-500' : ''} ${selectedRow?.id === s.id && isSidebarOpen ? 'bg-blue-50/40 dark:bg-blue-950/10 border-l-[#2383C2]' : ''}`}
                >
                  ...
                  <td style={tdStyle('descripcion')} className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-600 dark:text-gray-300">
                    {asignandoCodigoId === s.id ? (
                      <div className="relative w-full" ref={editDescRef} onClick={e => e.stopPropagation()}>
                        <input 
                          type="text"
                  ...
                  <td style={tdStyle('precioCosto')} className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70"><TruncCell value={`$${asignandoCodigoId === s.id ? (editFormData.precioCosto || '0') : (s.precioCosto || '0')}`} className="text-gray-600 dark:text-gray-300" /></td>
                  <td style={tdStyle('empresa')} className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70"><TruncCell value={asignandoCodigoId === s.id ? (editFormData.empresa || 'N/A') : (s.empresa || 'N/A')} className="text-gray-600 dark:text-gray-300" /></td>
                  ...
                  <td style={tdStyle('acciones')} className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-center">
                    {asignandoCodigoId === s.id ? (
                      <div className="flex justify-center gap-1" onClick={e => e.stopPropagation()}>
                        <button type="button" onClick={(e) => handleGuardarCodigoNuevo(s.refPath, e)} className="px-2 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-[10px] font-bold transition">
                          Guardar
                        </button>
                        <button type="button" onClick={(e) => { e.stopPropagation(); setAsignandoCodigoId(null); }} className="p-1 bg-gray-400 hover:bg-gray-500 text-white rounded transition">
                          <X size={12} />
                        </button>
                      </div>
                    ) : (
                      <div className="flex justify-center gap-1">
                        {s.estado === 'PENDIENTE_CODIGO' && !editandoId && (
                          <button type="button" onClick={(e) => handleIniciarAsignacion(s, e)} className="action-btn px-1.5 py-0.5 bg-amber-500 hover:bg-amber-600 text-white rounded text-[10px] font-bold flex items-center gap-1 transition" title="Asignar Código">
                            Asignar
                          </button>
                        )}
                        <button 
                          type="button" 
                          onClick={(e) => handleIniciarEdicionFormulario(s, e)} 
                          className={`action-btn p-1 rounded transition ${editandoId === s.id ? 'bg-amber-500 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
                          title="Editar en formulario principal"
                          disabled={!!asignandoCodigoId}
                        >
                          <Edit2 size={11} />
                        </button>
                        <button type="button" onClick={(e) => handleEliminarFila(s.refPath, e)} className="action-btn p-1 bg-red-600 hover:bg-red-700 text-white rounded transition" title="Eliminar Fila">
                          <Trash2 size={11} />
                        </button>
                      </div>
                    )}
      ...
      {/* BARRA LATERAL (SIDEBAR DE DETALLES Y ACCIONES DE FILA) */}
      <div 
        ref={sidebarRef}
        className={`fixed top-0 right-0 md:relative w-[340px] h-full bg-gray-50 dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 z-30 shadow-2xl md:shadow-none flex flex-col transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full md:hidden'}`}
      >
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-white dark:bg-gray-800">
          <div className="flex items-center gap-2 font-bold text-[13px] text-gray-700 dark:text-gray-200">
            <Sidebar size={16} className="text-[#2383C2]" />
            DETALLE DE REGISTRO
          </div>
          <button onClick={() => setIsSidebarOpen(false)} className="p-1 rounded text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
            <X size={16} />
          </button>
        </div>

        {selectedRow ? (
          <div className="p-4 flex-grow overflow-y-auto flex flex-col gap-4 text-[12px]">
            
            {/* Estado e Info rápida */}
            <div className="p-3 rounded bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-gray-400 font-medium">Estado actual:</span>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${selectedRow.estado === 'PENDIENTE_CODIGO' ? 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400'}`}>
                  {selectedRow.estado || 'INGRESADO'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400 font-medium">Modalidad:</span>
                <span className="font-semibold text-gray-700 dark:text-gray-300">{selectedRow.modalidad}</span>
              </div>
            </div>

            {/* Datos Quirúrgicos */}
            <div className="space-y-2">
              <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Info size={12}/> Datos Clínicos</h3>
              <div className="grid grid-cols-2 gap-2 bg-white dark:bg-gray-800 p-3 rounded border border-gray-100 dark:border-gray-700/50">
                <div><label className="text-gray-400 block text-[10px]">Admisión</label><span className="font-medium text-gray-800 dark:text-gray-200">{selectedRow.admision}</span></div>
                <div><label className="text-gray-400 block text-[10px]">F. Cirugía</label><span className="font-medium text-gray-800 dark:text-gray-200">{selectedRow.fechaCx}</span></div>
                <div className="col-span-2 border-t border-gray-100 dark:border-gray-700/50 pt-1.5"><label className="text-gray-400 block text-[10px]">Paciente</label><span className="font-medium text-gray-800 dark:text-gray-200">{selectedRow.paciente}</span></div>
                <div className="col-span-2 border-t border-gray-100 dark:border-gray-700/50 pt-1.5"><label className="text-gray-400 block text-[10px]">Médico</label><span className="font-medium text-gray-800 dark:text-gray-200">{selectedRow.medico}</span></div>
              </div>
            </div>

            {/* Datos Insumo / Código */}
            <div className="space-y-2">
              <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Info size={12}/> Detalles del Insumo</h3>
              <div className="bg-white dark:bg-gray-800 p-3 rounded border border-gray-100 dark:border-gray-700/50 space-y-2">
                <div>
                  <label className="text-gray-400 block text-[10px]">Descripción</label>
                ...
                {selectedRow.delivery && (
                  <div className="border-t border-gray-100 dark:border-gray-700/50 pt-2">
                    <label className="text-gray-400 block text-[10px]">Delivery</label>
                    <span className="font-medium text-gray-700 dark:text-gray-300">{selectedRow.delivery}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Auditoría */}
            <div className="mt-auto bg-gray-100 dark:bg-gray-800/40 p-3 rounded text-[11px] text-gray-400 space-y-1">
              <div><strong>Registrado por:</strong> {selectedRow.registradoPor || 'Usuario'}</div>
              <div><strong>F. Registro:</strong> {formatearFecha(selectedRow.fechaRegistro)}</div>
            </div>

            {/* Acciones Rápidas del Sidebar */}
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
              <button 
                onClick={(e) => handleIniciarEdicionFormulario(selectedRow, e)}
                className="h-8 bg-blue-600 hover:bg-blue-700 text-white rounded font-bold text-[11px] flex items-center justify-center gap-1.5 transition"
              >
                <Edit2 size={12} /> Editar Fila
              </button>
              <button 
                onClick={(e) => handleEliminarFila(selectedRow.refPath, e)}
                className="h-8 bg-red-600 hover:bg-red-700 text-white rounded font-bold text-[11px] flex items-center justify-center gap-1.5 transition"
              >
          ...
          <div className="flex-grow flex flex-col items-center justify-center p-6 text-center text-gray-400 dark:text-gray-500">
            <Sidebar size={32} className="mb-2 opacity-50 text-gray-300" />
            <span>Selecciona cualquier fila de la tabla para ver u operar sobre el desglose de datos detallado.</span>
          </div>
        )}
      </div>
