...
const PdfConsigna = () => {
  ...
  const parsePDF = async (file) => {
    ...
    return { folio, fchEmis, rznSoc, folioRef, detalles };
  };
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      ...
      console.error(error);
      showToast("Error al importar archivos PDF", "error");
    } finally {
      setCargando(false);
    }
  }, [userData, showToast]);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden p-0 relative">
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-gray-500/20 dark:bg-gray-950/40 backdrop-blur-[2px] transition-all duration-500">
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-8 rounded-3xl shadow-2xl border border-white/50 dark:border-gray-700/50 flex flex-col items-center gap-5 min-w-[280px]">
            ...
            <div className="text-center">
              <h3 className="text-[#2383C2] font-bold text-[15px] tracking-tight">Procesando archivos PDF</h3>
      ...
      <h2 className="text-[14px] font-bold text-gray-700 dark:text-gray-100 p-4 flex items-center gap-2 border-b border-gray-200 dark:border-gray-700">
        <Package size={16} className="text-[#2383C2]" /> CONSIGNACIÓN PDF
            ...
            <div className="p-6">
              ...
              >
                ...
                <h4 className="text-[14px] font-bold text-slate-700 dark:text-gray-200 mb-1">
                  {isDragActive ? "¡Suelta los archivos aquí!" : "Arrastra tus archivos PDF"}
                </h4>
                <p className="text-[12px] text-slate-400 dark:text-gray-400 max-w-[280px] leading-relaxed">
                  O haz clic para <span className="text-[#2383C2] font-semibold underline">explorar tu equipo</span>. Solo se admiten archivos .pdf
                </p>
              </div>
            </div>
