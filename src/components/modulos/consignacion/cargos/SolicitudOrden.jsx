...
import { collectionGroup, collection, doc, writeBatch, onSnapshot } from 'firebase/firestore';
import { db } from '../../../../firebaseConfig';
import { Truck, CheckSquare, Square, ChevronDown, ChevronRight, PackageOpen, Download } from 'lucide-react';
...
// Interpreta la fecha clínica del registro (fechaCx) para obtener año/mes.
// Soporta: Timestamp de Firestore, formato chileno DD-MM-AAAA (o con '/'),
// y formato ISO AAAA-MM-DD. Si no puede interpretarla, usa fallbackDate.
const extraerAnioMes = (valor, fallbackDate) => {
  try {
    if (!valor) throw new Error('vacio');

    // Firestore Timestamp con método toDate()
    if (typeof valor === 'object' && typeof valor.toDate === 'function') {
      const d = valor.toDate();
      return { anio: d.getFullYear().toString(), mes: String(d.getMonth() + 1).padStart(2, '0') };
    }
    // Firestore Timestamp plano { seconds, nanoseconds }
    ...
    if (partes.length === 3) {
      // Formato ISO: AAAA-MM-DD (el primer segmento tiene 4 dígitos)
      if (partes[0].length === 4) {
        return { anio: partes[0], mes: partes[1].padStart(2, '0') };
      }
      // Formato chileno: DD-MM-AAAA (el último segmento tiene 4 dígitos)
      if (partes[2].length === 4) {
        return { anio: partes[2], mes: partes[1].padStart(2, '0') };
      }
    }
    ...
    return {
      anio: fallbackDate.getFullYear().toString(),
      mes: String(fallbackDate.getMonth() + 1).padStart(2, '0')
    };
  }
};
...
const SolicitudOrden = () => {
  ...
  // Escuchar solicitudes en estado "SOLICITAR ORDEN"
  useEffect(() => {
    const qS = collection(db, "consignacion_solicitud");
    const unsubscribe = onSnapshot(qS,
      (snap) => {
        const data = snap.docs
          ...
          .filter(d => {
            if (!d.active || d.estado !== 'SOLICITAR ORDEN' || !d.codigo) return false;
  ...
  const handleExportarExcel = async () => {
    const itemsAExportar = selectedIds.length > 0 
    ...
    const filasExcel = [];
    // Guardamos junto a cada item sus detalles ya resueltos, para no recalcular al archivar
    const itemsConDetalles = [];

    itemsAExportar.forEach((s, index) => {
      const deliveryKey = s.delivery ? s.delivery.trim() : '';
      const guiaAsociada = deliveryKey ? guiasMap[deliveryKey] : null;
      const folioGuia = guiaAsociada ? guiaAsociada.folio : 'No Encontrada';

      // 1. Agregar Fila Principal
      filasExcel.push({
        "Tipo Registro": "PRINCIPAL",
        "N°": index + 1,
        "Admisión": s.admision || 'N/A',
        "Paciente": s.paciente || 'N/A',
        "Médico": s.medico || 'N/A',
        "Fecha": s.fecha || s.fechaCx || 'N/A',
        "Código": limpiarCodigo(s.codigo),
        "Descripción": s.descripcion || 'N/A',
        "Cantidad": s.cantidad || 0,
        "Precio Costo": s.precioCosto || 0,
        "Atributo": s.atributo || 'N/A',
        "Lote": s.lote || 'N/A',
        "F. Vencimiento": s.fechaVencimiento || 'N/A',
        "Delivery": s.delivery || 'N/A',
        "Guías (Folio)": folioGuia,
        "Referencia": s.referencia || 'N/A'
      });

      // 2. Procesar y agregar Subfilas de la Guía
      const detallesFiltrados = (guiaAsociada?.detalles || [])
        ...
        .map(det => {
          ...
          filasExcel.push({
            ...
            "Fecha": s.fecha || s.fechaCx || 'N/A',
            "Código": "No lleva OC",
            "Descripción": descripcionEncontrada,
            "Cantidad": det.cantidad || 0,
            "Precio Costo": 0,
            "Atributo": s.atributo || 'N/A',
            "Lote": det.dscItem || 'N/A',
            "F. Vencimiento": det.fchVenc || 'N/A',
            "Delivery": s.delivery || 'N/A',
            "Guías (Folio)": folioGuia,
            "Referencia": refSubfila
          });

          return { ...det, codigoLimpio: refSubfila, descripcionResuelta: descripcionEncontrada };
        });

      itemsConDetalles.push({ item: s, folioGuia, detallesFiltrados });
    });

    const worksheet = XLSX.utils.json_to_sheet(filasExcel);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Despachos y Guías");

    const maxCols = Object.keys(filasExcel[0] || {});
    worksheet["!cols"] = maxCols.map(col => ({
      wch: Math.max(...filasExcel.map(row => (row[col] ? row[col].toString().length : 10))) + 3
    }));

    XLSX.writeFile(workbook, `Reporte_Despachos_Delivery_${new Date().toISOString().slice(0, 10)}.xlsx`);
    showToast("Excel descargado correctamente.", "success");

    // 3. Traspasar los registros exportados a consignacion_historial/{año}/meses/{mes}/datos/{id}
    await archivarEnHistorial(itemsConDetalles);
  };

  const archivarEnHistorial = async (itemsConDetalles) => {
    if (itemsConDetalles.length === 0) return;

    setProcesandoAccion(true);
    try {
      const ahora = new Date(); // solo se usa como fallback y para "actualizadoEn"/"fechaExportacion"
      const batch = writeBatch(db);

      // Para no repetir el mismo set() en el mismo doc año/mes dentro del batch
      const aniosEscritos = new Set();
      const mesesEscritos = new Set();
      const combosUsados = new Set(); // para el mensaje final de éxito

      itemsConDetalles.forEach(({ item, folioGuia, detallesFiltrados }) => {
        if (!item.path) return;

        // Año/mes según la fecha clínica del registro (fechaCx), no la fecha de exportación
        const { anio, mes } = extraerAnioMes(item.fechaCx || item.fecha, ahora);
        combosUsados.add(`${anio}/${mes}`);

        // Doc "año": solo se escribe una vez por año dentro de este batch
        if (!aniosEscritos.has(anio)) {
          const anioRef = doc(db, "consignacion_historial", anio);
          batch.set(anioRef, { anio, actualizadoEn: ahora }, { merge: true });
          aniosEscritos.add(anio);
        }

        // Doc "mes": solo se escribe una vez por combinación año/mes dentro de este batch
        const claveMes = `${anio}-${mes}`;
        if (!mesesEscritos.has(claveMes)) {
          const mesRef = doc(db, "consignacion_historial", anio, "meses", mes);
          batch.set(mesRef, { anio, mes, actualizadoEn: ahora }, { merge: true });
          mesesEscritos.add(claveMes);
        }
        ...
        const datosHistorial = {
          ...
          folioGuiaAsociada: folioGuia,
          subfilasGuia: detallesFiltrados
        };
        delete datosHistorial.id;
        delete datosHistorial.path;

        // consignacion_historial/{año}/meses/{mes}/datos/{id}
        const destinoRef = doc(db, "consignacion_historial", anio, "meses", mes, "datos", item.id);
        batch.set(destinoRef, datosHistorial);

        // Se elimina de consignacion_solicitud
        const origenRef = doc(db, item.path);
        batch.delete(origenRef);
      });

      await batch.commit();
      const resumenCombos = Array.from(combosUsados).join(', ');
      showToast(`${itemsConDetalles.length} registro(s) archivados en historial (${resumenCombos}).`, "success");
      setSelectedIds([]);
    } catch (error) {
      console.error("Error al archivar en consignacion_historial:", error);
      showToast("El Excel se descargó, pero no se pudo archivar el historial.", "error");
    } finally {
    ...
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden relative transition-colors">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-2 bg-gray-50/50 dark:bg-gray-900/20">
        <h2 className="text-[14px] font-bold text-gray-700 dark:text-gray-200 flex items-center gap-2">
          ...
          DESPACHO / DELIVERY (SOLICITAR ORDEN)
        </h2>
        ...
        <div className="flex items-center gap-2">
          {solicitudes.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold text-[11px] uppercase tracking-wider px-3 py-1.5 rounded shadow-sm transition-all active:scale-95"
              title="Descargar Excel y archivar los registros en el historial"
            >
              <Download size={12} />
              Exportar y Archivar
            </button>
                    ...
                    {key === 'select' ? (
                      ...
                      <>
                        ...
                        <div
                          ...
                          className="absolute top-0 right-0 h-full w-1.5 cursor-col-resize hover:bg-amber-500/60 active:bg-amber-50 z-20 transition-colors"
                        />
                      </>
                    )}
                  </th>
            ...
            {solicitudes.length === 0 ? (
              <tr>
                <td colSpan={COL_KEYS.length} className="p-8 text-center text-gray-400 italic text-[13px]">
                  No hay solicitudes pendientes en estado SOLICITAR ORDEN.
                </td>
              </tr>
            ) : (
