...
import { User, Plus, Trash2, Search, RefreshCw, Pencil, Save, X } from 'lucide-react';
...
const Medicos = () => {
  ...
  const [formData, setFormData] = useState({ nombre: '', especialidad: '', estado: 'ACTIVO' });
  ...
  const PATH_VISTA = "/maestros/medicos";

  useEffect(() => {
    const q = query(collection(db, "maestros_medicos"), orderBy("fechaRegistro", "asc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setMedicos(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "maestros_medicos", editingId), formData);
        showToast("Médico actualizado correctamente", "success");
      } else {
        await addDoc(collection(db, "maestros_medicos"), {
          ...formData,
      ...
      setFormData({ nombre: '', especialidad: '', estado: 'ACTIVO' });
      setEditingId(null);
    } catch (error) {
  ...
  const handleDelete = (id) => {
    confirmAction(
      ...
      async () => {
        try {
          await deleteDoc(doc(db, "maestros_medicos", id));
          showToast("Médico eliminado correctamente", "info");
        } catch (error) {
  ...
  const iniciarEdicion = (m) => {
    setEditingId(m.id);
    setFormData({ nombre: m.nombre, especialidad: m.especialidad, estado: m.estado || 'ACTIVO' });
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden p-0">
      <h2 className="text-[14px] font-bold text-gray-700 dark:text-gray-100 p-4 flex items-center gap-2 border-b border-gray-200 dark:border-gray-700">
        <User size={16} className="text-[#2383C2]" /> {editingId ? "EDITAR MÉDICO" : "REGISTRO DE MÉDICOS"}
      </h2>
        ...
        <form onSubmit={handleGuardar} className="p-4 flex flex-wrap items-end gap-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-800/20">
          ...
          {hasPermission(PATH_VISTA, "formulario_registro", "input_especialidad") && (
            <div className="w-[200px]">
              <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Especialidad</label>
              <input value={formData.especialidad} onChange={e => setFormData({ ...formData, especialidad: e.target.value })} className="w-full h-8 px-2 border border-gray-300 dark:border-gray-600 rounded text-[12px] outline-none focus:border-[#2383C2] dark:focus:border-[#2383C2] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100" />
            </div>
          )}
          ...
          {editingId && (
            <button type="button" onClick={() => { setEditingId(null); setFormData({ nombre: '', especialidad: '', estado: 'ACTIVO' }) }} className="h-8 px-4 bg-gray-200 dark:bg-gray-700 rounded font-bold text-[12px] text-gray-600 dark:text-gray-300 flex items-center gap-2 hover:bg-gray-300 dark:hover:bg-gray-600 transition">
              <X size={14} /> Cancelar
            </button>
          )}
        </form>
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 flex justify-between items-center border-b border-gray-200 dark:border-gray-700">
          {hasPermission(PATH_VISTA, "barra_busqueda", "input_buscar") && (
            <div className="relative w-72">
              ...
              <input value={busqueda} onChange={e => setBusqueda(e.target.value)} className="w-full h-8 pl-8 pr-2 border border-gray-300 dark:border-gray-600 rounded text-[12px] outline-none bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:border-[#2383C2] dark:focus:border-[#2383C2]" placeholder="Buscar..." />
            </div>
          )}
        </div>
      )}
          ...
          <table className="w-full text-left text-[12px] border-collapse">
            <thead className="bg-gray-100 dark:bg-gray-900 sticky top-0 z-10">
              <tr className="text-gray-600 dark:text-gray-400 uppercase font-bold text-[11px]">
                ...
                {hasPermission(PATH_VISTA, "tabla_datos", "col_especialidad") && <th className="p-3 border-b border-r border-gray-200 dark:border-gray-700">Especialidad</th>}
            ...
            <tbody>
              {medicosFiltrados.map((m, index) => (
                <tr key={m.id} className="border-l-4 border-transparent hover:border-[#2383C2] hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors">
                  ...
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_nombre") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-700 dark:text-gray-200 font-medium">{m.nombre}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_especialidad") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-600 dark:text-gray-300">{m.especialidad}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_estado") && (
                    <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${m.estado === 'INACTIVO' ? 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400' : 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-400'}`}>
                        {m.estado || 'ACTIVO'}
                      </span>
                    </td>
                  )}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_registrador") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{m.registradoPor || 'N/A'}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_fecha") && <td className="p-3 border-b border-r border-gray-200 dark:border-gray-700/70 text-gray-500 dark:text-gray-400">{formatearFecha(m.fechaRegistro)}</td>}
                  ...
                  <td className="p-3 border-b border-gray-200 dark:border-gray-700 text-center">
                    <div className="flex justify-center gap-3">
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_editar") && (
                        <button onClick={() => iniciarEdicion(m)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition"><Pencil size={15} /></button>
                      )}
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_eliminar") && (
                        <button onClick={() => handleDelete(m.id)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition"><Trash2 size={15} /></button>
                      )}
                    </div>
                  </td>
                </tr>
