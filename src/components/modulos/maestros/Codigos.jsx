      ...
      {hasPermission(PATH_VISTA, "formulario_registro") && (
        <form onSubmit={handleGuardar} className="p-4 flex flex-wrap items-end gap-4 border-b border-gray-200 dark:border-gray-700">
          ...
          {hasPermission(PATH_VISTA, "formulario_registro", "select_tipo") && (
            <div className="w-[120px]"><label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Tipo</label><select value={formData.tipo} onChange={e => setFormData({ ...formData, tipo: e.target.value })} className={baseInputClass}><option value="IMPLANTE" className="dark:bg-gray-900">IMPLANTE</option><option value="INSUMO" className="dark:bg-gray-900">INSUMO</option><option value="PAD" className="dark:bg-gray-900">PAD</option></select></div>
          )}
