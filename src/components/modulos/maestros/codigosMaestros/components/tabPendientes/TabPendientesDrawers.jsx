          ...
          <div className="truncate">
            ...
            <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate">
              ...
              <span className="font-semibold text-gray-700 dark:text-gray-300">
                {selectedPendiente?.referencia}
              </span>{' '}
              ({selectedPendiente?.empresa})
            </p>
          </div>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
        {loadingLogs ? (
          ...
          <div className="text-center py-12 text-gray-400 dark:text-gray-500 text-[10px]">
            No hay registros de auditoría para este pendiente.
          </div>
        ) : (
          logsList.map((log) => (
            <div
                ...
                <span
                  className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                    log.accion === 'CREACION' || log.accion === 'CREACION_MASIVA'
                      ...
                      : log.accion === 'EDICION' || log.accion === 'ACTUALIZACION_PRECIO_IMPORTACION'
              ...
              <div className="text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700/60 text-[10px]">
                {(log.accion === 'CREACION' || log.accion === 'CREACION_MASIVA') && (
                  <div className="space-y-0.5">
                    ...
                    <p><strong>Descriptor Empresa:</strong> {log.detalles?.descriptorEmpresa || 'N/A'}</p>
                    <p><strong>Tipo:</strong> {log.detalles?.tipo || 'N/A'}</p>
                    <p><strong>Segmento:</strong> {log.detalles?.segmento || 'N/A'}</p>
                    <p><strong>Clase:</strong> {log.detalles?.clase || 'N/A'}</p>
                    <p><strong>Precio Neto:</strong> $ {formatearPrecioLog(log.detalles?.precioNeto)}</p>
                    <p><strong>CX:</strong> {log.detalles?.cx || 'N/A'}</p>
                    <p><strong>Observación:</strong> {log.detalles?.observacion || 'N/A'}</p>
                    <p>
                      <strong>Método:</strong>{' '}
                      {log.detalles?.metodoRegistro === 'IMPORTACION' ? 'Importación' : 'Manual'}
                    </p>
                  </div>
                )}
                ...
                {log.accion === 'ASIGNACION_CODIGO' && (
                  <div className="space-y-0.5">
                    <p className="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                      <Tag size={12} /> Código Definitivo Asignado: {log.detalles?.codigoAsignado}
                    </p>
                  </div>
                )}
                ...
                {log.accion === 'EDICION' && (
                  <ul className="space-y-0.5">
                    ...
                    {log.detalles?.empresa !== undefined && (
                      <li><strong>Empresa:</strong> {log.detalles.empresa}</li>
                    )}
                    {log.detalles?.descriptorEmpresa !== undefined && (
                      <li><strong>Descriptor Empresa:</strong> {log.detalles.descriptorEmpresa || 'Vacío'}</li>
                    )}
                    {log.detalles?.tipo !== undefined && (
                      <li><strong>Tipo:</strong> {log.detalles.tipo || 'Vacío'}</li>
                    )}
                    {log.detalles?.segmento !== undefined && (
                      <li><strong>Segmento:</strong> {log.detalles.segmento || 'Vacío'}</li>
                    )}
                    {log.detalles?.clase !== undefined && (
                      <li><strong>Clase:</strong> {log.detalles.clase || 'Vacío'}</li>
                    )}
                    {log.detalles?.precioNeto !== undefined && (
                      <li><strong>Precio Neto:</strong> $ {formatearPrecioLog(log.detalles.precioNeto)}</li>
                    )}
                    {log.detalles?.cx !== undefined && (
                      <li><strong>CX:</strong> {log.detalles.cx || 'Vacío'}</li>
                    )}
                    {log.detalles?.observacion !== undefined && (
                ...
                {log.accion === 'ELIMINACION' && (
                  <p className="text-red-500 font-medium">
                    Registro eliminado ({log.detalles?.referencia} - {log.detalles?.empresa})
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        <div className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50/50 dark:bg-gray-900/30 space-y-2">
          ...
          <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed">
            Descarga la lista actual de registros pendientes ({totalPendientes}{' '}
