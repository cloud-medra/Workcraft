                ...
                <tr key={e.id} className="border-l-4 border-transparent hover:border-[#2383C2] hover:bg-gray-50">
                  ...
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_estado") && (
                    <td className="p-3 border-b border-r border-gray-200">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${e.estado === 'INACTIVO' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                        {e.estado || 'ACTIVO'}
                      </span>
                    </td>
                  )}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_registrador") && <td className="p-3 border-b border-r border-gray-200 text-gray-500">{e.registradoPor || 'N/A'}</td>}
                  {hasPermission(PATH_VISTA, "tabla_datos", "col_fecha") && <td className="p-3 border-b border-r border-gray-200 text-gray-500">{formatearFecha(e.fechaRegistro)}</td>}

                  <td className="p-3 border-b border-gray-200 text-center">
                    <div className="flex justify-center gap-3">
                      {hasPermission(PATH_VISTA, "tabla_datos", "action_editar") && (
                        <button onClick={() => iniciarEdicion(e)} className="text-blue-600 hover:text-blue-800"><Pencil size={15} /></button>
                      )}
